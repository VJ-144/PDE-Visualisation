...
def PlotCorrel(prob):
    ...
    # prob = data[:,0]
    radius = np.linspace(0, 26, 25)

    # setting up plot figure/titles/labels
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))

    ax.set_title(fr'Correlation Proabilites for {N}x{N} Matrix', pad=16)
    ax.errorbar(radius, prob, marker='o', markersize = 4, linestyle='--', color='red')
...
def PlotPoints(data):
    ...
    popt, pcov = curve_fit(sine, time, a1, p0=[0.8, 0.2, 10, 2])

    xfit = np.linspace(0, 200, 200)
    
    ax2.errorbar(time, a1, marker='o', markersize = 4, linestyle='--', color='black', label='point 1')
    ax2.errorbar(xfit, sine(xfit, *popt), marker='', markersize = 4, linestyle='-', color='orange', label='Sin Fit')
    ...
    period = np.round(1/frequency, 2)

    print(f'Period={period}')
    ax2.set_title(fr'Best Fit for A Species Point, Period={period}')
    # saving to plot to file
...
def main():
    ...
    # data = np.loadtxt("Data/Tao_Mat_50N_D0.5_q1.0_p2.5.txt")
    # data = np.loadtxt("Data/PointsTao_50N_D0.5_q1.0_p2.5.txt")
    data = np.loadtxt('correlationData.txt')
